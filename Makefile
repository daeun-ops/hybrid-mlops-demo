# Makefile that doesn't require TABs for recipes
.RECIPEPREFIX := >
...
> for i in {1..45}; do
>   s=$$(docker inspect -f '{{.State.Health.Status}}' ray-inference 2>/dev/null || echo unknown)
>   if [ "$$s" = "healthy" ]; then echo "✅ Container is healthy."; break; fi
>   sleep 2
> done
> $(MAKE) infer
